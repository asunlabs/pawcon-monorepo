...
//         process.env.CONSTRUCTOR.CURIOUSPAWONEER.churuAddr.HARDHAT,
//         process.env.CONSTRUCTOR.CURIOUSPAWONEER.nonce,
//         process.env.CONSTRUCTOR.CURIOUSPAWONEER.cid
//     )
//     // deploy CP and attach it to address 19
//     await (
//         await curiousPawoneer.deployed()
//     ).attach(key.HARDHATNETWORK.account19)
// })

describe('CuriousPawoneer.sol', async function () {
    // Test case 1 : PASS
    it('Should return contract name : Curious Pawoneer', async function () {
        assert('CuriousPawoneer' === 'CuriousPawoneer');
    });
    ...
    it.skip('Token ID should be unique', async function () {
        ...
        // expect(await curiousPawoneer.ownerOf(1)).not.to.equal(
        //   key.HARDHATNETWORK.account1
        // )
    });
});
